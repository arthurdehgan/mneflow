...
def _make_example(X, y, n, target_type="int"):
    ...
    feature["n"] = tf.train.Feature(int64_list=tf.train.Int64List(value=n.flatten()))
...
def _split_indices(X, y, n_folds=5):
    ...
    subset_proportion = 1.0 / float(n_folds)
...
def import_data(inp, picks=None, array_keys={"X": "X", "y": "y"}):
    ...
    if isinstance(inp, (mne.epochs.EpochsFIF, mne.epochs.BaseEpochs)):
        ...
        fname = inp
        if fname[-3:] == "fif":
            epochs = mne.epochs.read_epochs(fname, preload=True, verbose="CRITICAL")
            print(np.unique(epochs.events[:, 2]))
            events = epochs.events[:, 2]
            epochs.crop(tmin=-1.0, tmax=1.0)
    ...
    if isinstance(picks, (np.ndarray, list, tuple)):
        ...
        if np.any(data.shape[1] <= picks):
            raise ValueError(
                "Invalid picks {} for n_channels {} ".format(
    ...
    if overwrite or not os.path.exists(savepath + out_name + "_meta.pkl"):
        ...
        for inp in inputs:
            ...
            if np.any(data) == None:
                ...
                print(
                    "Input shapes: X (n, ch, t) : ",
                    data.shape,
                    "y (n, [signal_channels], y_shape) : ",
                    events.shape,
                    "\n",
                    "input_type : ",
                    input_type,
                ...
                Y = preprocess_targets(
                    Y, scale_y=scale_y, transform_targets=transform_targets
                )

                if target_type == "int":
                    Y, n_ev, meta["class_ratio"], meta["orig_classes"] = produce_labels(
                        Y
                    )
                    Y = _onehot(Y)

                if test_set == "holdout":
                    X, Y, x_test, y_test, test_fold = _split_sets(
                        X, Y, folds=folds, sample_counter=meta["train_size"]
                    )
                ...
                meta["train_paths"].append(
                    "".join([savepath, out_name, "_train_", str(jj), ".tfrecord"])
                )

                _write_tfrecords(
                    X, Y, n, meta["train_paths"][-1], target_type=target_type
                )

                if test_set == "loso":
                    meta["test_size"] = len(Y)
                    meta["test_paths"].append(
                        "".join([savepath, out_name, "_test_", str(jj), ".tfrecord"])
                    )
                    _write_tfrecords(
                        X, Y, n, meta["test_paths"][-1], target_type=target_type
                    )
...
def produce_labels(y, return_stats=True):
    ...
    """
    classes, inds, inv, counts = np.unique(
...
def _segment(
    data, segment_length=200, seq_length=None, stride=None, input_type="trials"
):
...
def preprocess_realtime(data, decimate=False, picks=None, bp_filter=False, fs=None):
    """
...
def preprocess(
    ...
    if scale:
        data = scale_to_baseline(
            data, baseline=scale_interval, crop_baseline=crop_baseline
        )

    # define folds
    if input_type == "continuous":
        data, events, folds = cont_split_indices(
            data, events, n_folds=5, segments_per_fold=10
        )
    ...
    assert (
        X.shape[0] == Y.shape[0]
    ), "n_epochs in X ({}) does not match n_epochs in Y ({})".format(
        X.shape[0], Y.shape[0]
    )
...
def cosine_similarity(y_true, y_pred):
    ...
    return np.dot(y_pred.T, y_true) / (
        np.sqrt(np.sum(y_pred**2, axis=0)) * np.sqrt(np.sum(y_true**2, axis=0))
    )
