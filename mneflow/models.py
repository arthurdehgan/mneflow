...
class BaseModel:
    ...
    def build(
        self, optimizer="adam", loss=None, metrics=None, mapping=None, learn_rate=3e-4
    ):
        """Compile a model.
        ...
        self.params = {
            "optimizer": tf.optimizers.get(optimizer).from_config(
        ...
        if self.dataset.h_params["target_type"] in ["float", "signal"]:
            ...
            self.params.setdefault(
                "loss",
                [tf.keras.losses.CategoricalCrossentropy(from_logits=True, name="cce")],
            )
            self.params.setdefault(
    ...
    def build_graph(self):
        ...
        self.fc = Dense(size=self.out_dim, nonlin=tf.identity, specs=self.specs)
        y_pred = self.fc(flat)
        # y_pred = fc_1
    ...
    def train(
        ...
        if class_weights:
            multiplier = 1.0 / min(class_weights.values())
        ...
        if mode == "single_fold":
            ...
            if collect_patterns and self.scope == "lfcnn":
                self.collect_patterns(
                    fold=0, n_folds=n_folds, n_comp=int(collect_patterns)
                )
            ...
            for jj in range(n_folds):
                ...
                if collect_patterns and self.scope == "lfcnn":
                    self.collect_patterns(
                        fold=jj, n_folds=n_folds, n_comp=int(collect_patterns)
                    )
            ...
            print(
                "{} with {} folds completed. Loss: {:.4f} +/- {:.4f}. Metric: {:.4f} +/- {:.4f}".format(
                    mode,
                    n_folds,
                    np.mean(losses),
                    np.std(losses),
            ...
            for jj in range(n_folds):
                ...
                test = self.dataset._build_dataset(
                    test_subj, test_batch=None, split=False
                )
                ...
                print(
                    "Subj: {} Loss: {:.4f}, Metric: {:.4f}".format(
                        jj, loso_loss, loso_metric
                    )
                )
    ...
    def prune_weights(self, increase_regularization=3.0):
        stop_early = tf.keras.callbacks.EarlyStopping(
        ...
        self.t_hist_p = self.km.fit(
            self.dataset.train,
            validation_data=self.dataset.val,
            epochs=30,
            steps_per_epoch=self.train_params[1],
            shuffle=True,
            validation_steps=self.dataset.validation_steps,
            callbacks=[stop_early],
            verbose=2,
    ...
    def update_log(self, rms=None, prefix=""):
        ...
        if len(self.dataset.h_params["test_paths"]) > 0 and log["mode"] != "loso":
            ...
            print(
                "Updating log: test loss: {:.4f} test metric: {:.4f}".format(
                    t_loss, t_metric
                )
            )
            if self.dataset.h_params["target_type"] == "float":
        ...
        with open(
            self.model_path + self.scope + "_log.csv", "a+", newline=""
        ) as csv_file:
    ...
    def save(self):
        ...
        if hasattr(self, "cv_patterns"):
            ...
            print(
                ...
                + self.model_path
                + self.model_name
                + "\\mneflow_patterns.npz"
            )
            np.savez(
                self.model_path + self.model_name + "\\mneflow_patterns.npz",
    ...
    def predict_sample(self, x):
        ...
        assert x.shape[-2:] == (
            n_t,
            n_ch,
        ), "Shape mismatch! Expected {}x{}, \
            got {}x{}".format(
            n_t, n_ch, x.shape[-2], x.shape[-1]
        )
    ...
    def predict(self, dataset=None):
        ...
        if not dataset:
            ...
            dataset = self.dataset._build_dataset(
                dataset, split=False, test_batch=None, repeat=False
            )
    ...
    def evaluate(self, dataset=False):
        ...
        if not dataset:
            ...
            dataset = self.dataset._build_dataset(
                dataset, split=False, test_batch=None, repeat=True
            )
        elif not isinstance(dataset, tf.data.Dataset):
            print("Specify dataset")
            return None, None

        losses, metrics = self.km.evaluate(
            dataset, steps=self.dataset.validation_steps, verbose=0
        )
        return losses, metrics

    def plot_confusion_matrix(
        self, cm=None, classes=None, normalize=False, title=None, cmap=plt.cm.Blues
    ):
        """
...
class LFCNN(BaseModel):
    ...
    def __init__(self, Dataset=None, specs=dict(), meta=None):
        ...
        specs.setdefault("l1_lambda", 0.0)
        specs.setdefault("l2_lambda", 0.0)
    ...
    def build_graph(self):
        ...
        """
        self.dmx = DeMixing(
            size=self.specs["n_latent"], nonlin=tf.identity, axis=3, specs=self.specs
        )
        ...
        self.fin_fc = Dense(size=self.out_dim, nonlin=tf.identity, specs=self.specs)
    ...
    def build_encoder(self):
        ...
        self.enc_fc = Dense(
            size=self.fin_fc.w.shape[0], nonlin=tf.identity, specs=self.specs
        )
        enc_tconv_activations = self.enc_fc(self.y_pred)
        enc_tconv_activations_r = tf.reshape(
            enc_tconv_activations,
            [-1, 1, self.pooled.shape[-2], self.specs["n_latent"]],
        )
        print(enc_tconv_activations_r.shape)
        enc_dropout = Dropout(self.specs["dropout"], noise_shape=None)(
            enc_tconv_activations_r
        )
        ...
        self.params["enc_loss"] = [
            tf.keras.losses.MAE
        ]  # tf.reduce_mean((self.X_pred - self.inputs)**2)
        # self.params['enc_metrics'] = tf.keras.metrics.RootMeanSquaredError(name="RMSE")

        # print(params)
        self.km_enc.compile(
            optimizer=tf.keras.optimizers.legacy.Adam(), loss=self.params["enc_loss"]
        )
    ...
    def compute_enc_patterns(self, inputs=None):
        ...
        pooled = tf.reshape(
            self.enc_fc(inputs),
            [self.out_dim, 1, self.pooled.shape[-2], self.specs["n_latent"]],
        )
    ...
    def patterns_cov_xx(self, y, weights, activations, dcov):
        ...
        assert X.shape[-1] == w.shape[0], "Shape mismatch X:{} w:{}".format(
            X.shape, w.shape
        )
        assert y.shape[-1] == w.shape[1], "Shape mismatch y:{} w:{}".format(
            y.shape, w.shape
        )
        ...
        for i in range(y.shape[-1]):
            ...
            patterns.append(np.einsum("hi, ij, jk -> h", dc, weights["dmx"], prec_sx))
        patterns = np.stack(patterns, -1)
        return patterns

    def patterns_cov_xy_hat(self, X, y, activations, weights):
        Sx_tconv = self.backprop_fc(
            activations["tconv"], activations["fc"], y, weights["out_weights"]
        )
        Sx_dmx = self.backprop_covxy(X, activations["dmx"], Sx_tconv, weights["dmx"])
    ...
    def backprop_fc(self, X, y_hat, y, w):
        ...
        assert y_hat.shape[-1] == w.shape[1], "Shape mismatch y:{} w:{}".format(
            y_hat.shape, w.shape
        )
        X = X - X.mean(0, keepdims=True)
        ...
        # compute directions of Sx: a = cov_xy*(cov_yy)**-1
        a_out = np.einsum(
            "jk, kl -> jl", cov_xy, prec_yy
        )  # shape = [n_t_pooled, n_latent, n_classes]
    ...
    def patterns_pinv_w(self, y, weights, activations, dcov):
        combined_topos = []
        pinv_dmx = np.linalg.pinv(
            weights["dmx"]
        ).T  # np.dot(spatial_filters, np.linalg.inv(np.dot(spatial_filters.T, spatial_filters)))
        pinv_wfc = np.linalg.pinv(
            weights["out_w_flat"]
        ).T  # np.dot(out_w_flat, np.linalg.inv(np.dot(out_w_flat.T, out_w_flat)))
        pinv_tck = np.linalg.pinv(
            weights["tconv"]
        ).T  # np.dot(tconv_kernels, np.linalg.inv(np.dot(tconv_kernels.T, tconv_kernels)))
        ...
        for class_y in range(self.out_dim):
            ...
            combined_topos.append(np.einsum("hi,ij,tj->ht", dc, weights["dmx"], Sxm))
    ...
    def patterns_wfc_mean(self, y, weights, activations, dcov):
        ...
        for class_y in range(self.out_dim):
            ...
            class_ind = tf.squeeze(tf.where(tf.argmax(y, 1) == class_y))  # [0]
            fc_bp_out = (
                np.dot(activations["fc"].numpy()[class_ind, :], weights["out_w_flat"].T)
            ).mean(0)

            fc_bp_out = fc_bp_out.reshape(
                [activations["tconv"].shape[2], activations["tconv"].shape[3]],
                order="C",
            )
            dc = dcov["class_conditional"][class_y]
            # fc_bp_out = np.maximum(fc_bp_out, 0)
            class_patterns = np.dot(dc, weights["dmx"])
    ...
    def compute_patterns(self, data_path=None):
        ...
        if not data_path:
            ...
            ds = self.dataset._build_dataset(
                data_path, split=False, test_batch=None, repeat=True
            )
        ...
        patterns_struct = {
            "weights": {"dmx": [], "tconv": [], "fc": [], "tconv_freq_resposes": {}},
            "ccms": {"dmx": [], "tconv": [], "fc": [], "input": [], "dmx_psd": []},
            "dcov": {"input_spatial": [], "class_conditional": [], "k-1": []},
            "patterns": {},
            "spectra": {},
        }
        ...
        (
            ...
            dcov["k-1"],
        ) = self._get_class_conditional_spatial_covariance(X, y)
        ...
        if self.dataset.h_params["target_type"] == "float":
            ...
            evokeds = np.array([X.numpy()[y_int == i, ...].mean(0) for i in y_unique])
        ...
        for class_y in range(self.out_dim):
            ...
            Sx_fc_ccm.append(
                activations["fc"].numpy()[class_ind, ...].mean(0, keepdims=True)
            )
        ...
        patterns["pinv_w"] = self.patterns_pinv_w(y, weights, activations, dcov).mean(
            -1
        )
        ...
        # compute the effect of removing each latent component on the cost function
        patterns_struct["compwise_loss"] = self.compute_componentwise_loss(
            X, y, weights
        )
    ...
    def collect_patterns(
        ...
        n_folds=1,
        n_comp=1,
        methods=["weight", "compwise_loss", "l2", "abs_weight", "output_corr"],
    ):
        """
        ...
        if not hasattr(self, "cv_patterns") or fold == 0:
            ...
            for method in methods:
                # n_folds = len(self.dataset.h_params['folds'][0])
                self.cv_patterns[method]["spatial"] = np.zeros(
                    [n_ch, self.y_shape[0], n_folds]
                )
                self.cv_patterns[method]["temporal"] = np.zeros(
                    [self.nfft, self.y_shape[0], n_folds]
                )
                self.cv_patterns[method]["psds"] = np.zeros(
                    [self.nfft, self.y_shape[0], n_folds]
                )
        for method in methods:
            if method not in combined_methods:
                # collect spatial patterns for 'weight' sorting
                topo, spectra, psds = self.single_component_pattern(
                    patterns_struct, sorting=method, n_comp=n_comp
                )
                ...
                self.cv_patterns[method]["spatial"][:, :, fold] = patterns_struct[
                    "patterns"
                ][method]
    ...
    def get_spectra(self, weights, activations, nfft=128):
        ...
        for i, flt in enumerate(weights["tconv"].T):
            ...
            ltc = activations["dmx"][:, 0, :, i] - np.mean(
                activations["dmx"][:, 0, :, i], axis=1, keepdims=True
            )
            fr, psd = welch(
                ltc, fs=self.dataset.h_params["fs"], nfft=nfft * 2, nperseg=nfft
            )
            if len(fr[:-1]) < nfft:
                nfft = len(fr[:-1])
    ...
    def get_weights(self):
        ...
        weights["out_weights"] = np.reshape(
            self.fin_fc.w.numpy(),
            [self.pooled.shape[2], self.dmx.size, self.out_dim],
            order="C",
        )
    ...
    def plot_evoked_peaks(
        self, data=None, t=None, class_subset=None, sensor_layout="Vectorview-mag"
    ):
        """
        ...
        assert ed.ndim == 2
        topoplot = self.plot_topos(
            ed, sensor_layout=sensor_layout, class_subset=class_subset
        )
    ...
    def make_fake_evoked(self, topos, sensor_layout):
        if "info" not in self.dataset.h_params.keys():
            ...
            info = create_info(lo.names, 1.0, sensor_layout.split("-")[-1])
            ...
            for i, ch in enumerate(lo.names):
                if info["chs"][i]["ch_name"] == ch:
                    ...
                    print(
                        "Channel name mismatch. info: {} vs lo: {}".format(
    ...
    def plot_topos(self, topos, sensor_layout="Vectorview-mag", class_subset=None):
        ...
        if class_subset is None:
            class_subset = np.arange(0, n, 1.0)

        fake_evoked = self.make_fake_evoked(topos_new, sensor_layout)

        ft = fake_evoked.plot_topomap(
            times=class_subset,
            colorbar=True,
            scalings=1,
            time_format="Class %g",
    ...
    def explore_components(
        ...
        def _onclick_component(event):
            ...
            f1.suptitle("Class: {}  Component: {}".format(class_ind, component_ind))

            a = np.dot(
                patterns_struct["dcov"]["class_conditional"][class_ind],
                patterns_struct["weights"]["dmx"],
            )

            self.fake_evoked_interactive.data[:, component_ind] = a[:, component_ind]
            self.fake_evoked_interactive.plot_topomap(
                times=[component_ind],
                axes=ax[0, 0],
                colorbar=False,
            ...
            # ax[1,].clear()
            ax[0, 1].semilogy(
                patterns_struct["spectra"]["freqs"], psd, label="Input RPS"
            )
            ax[0, 1].semilogy(
                patterns_struct["spectra"]["freqs"], out_psd, label="Output RPS"
            )
            # ax[0,1].semilogy(patterns_struct['spectra']['freqs'], freq_response,
            #             label='Freq_response')
            ax[0, 1].set_xlim(1, 125.0)
            ax[0, 1].set_ylim(1e-6, 1.0)
        ...
        if sorting == "weight_corr":
            F = (
                patterns_struct["weights"]["out_weights"]
                * patterns_struct["corr_to_output"]
            )
        ...
        topos = np.dot(
            patterns_struct["dcov"]["input_spatial"], patterns_struct["weights"]["dmx"]
        )
        psds = patterns_struct["spectra"]["psds"]
        ...
        r = [
            ptch.Rectangle(
                (i - 0.5, ind - 0.5), width=1, height=1, angle=0.0, facecolor="none"
            )
            for i, ind in enumerate(inds)
        ]

        pc = collections.PatchCollection(
            r, facecolor="none", alpha=0.5, edgecolor="red"
        )
        # ax = f.gca()
    ...
    def plot_waveforms(
        ...
        if np.any(self.uorder):
            ...
            if apply_kernels:
                scaled_waveforms = np.array(
                    [
                        np.convolve(kern, wf, "same")
                ...
                scaled_waveforms = (waveforms - waveforms.mean(-1, keepdims=True)) / (
                    2 * waveforms.std(-1, keepdims=True)
                )
            if bp_filter:
                scaled_waveforms = scaled_waveforms.astype(np.float64)
                scaled_waveforms = filter_data(
                    scaled_waveforms,
            ...
            [
                ax[0, 0].plot(times, wf, color="tab:grey", alpha=0.25)
                for i, wf in enumerate(scaled_waveforms)
            ...
            if not hasattr(self, "pattern_weights"):
                pattern_weights = np.einsum(
                    "ijk, jkl ->ikl", self.tc_out, self.out_weights
                )
                self.pattern_weights = np.maximum(
                    pattern_weights + self.out_biases[None, :], 0.0
                ).mean(0)
            ...
            r = [
                ptch.Rectangle((i, uo), width=1, height=1, angle=0.0)
                for i, uo in enumerate(self.uorder)
            ]
            pc = collections.PatchCollection(
                r, facecolor=None, alpha=0.5, linewidth=2.0, edgecolor="tab:orange"
            )
    ...
    def single_component_pattern(
        self, patterns_struct, sorting="compwise_loss", n_comp=1
    ):
        ...
        for i, comps in enumerate(order):
            ...
            c_psds.append(
                patterns_struct["spectra"]["psds"][comps, :].T,
            )
    ...
    def plot_combined_pattern(
        self,
        method="combined",
        sensor_layout=None,
        names=None,
        n_comp=1,
        plot_true_evoked=False,
    ):
        if not names:
        ...
        if hasattr(self, "cv_patterns"):
            ...
            topos = np.mean(self.cv_patterns[method]["spatial"], -1)
            filters = np.mean(self.cv_patterns[method]["temporal"], -1)
            psds = np.mean(self.cv_patterns[method]["psds"], -1)
            # freqs = self.freqs
            ...
            topos, filters, psds = self.single_pattern(sorting=method, n_comp=n_comp)

        freqs = self.freqs

        topos /= np.maximum(topos.std(axis=0, keepdims=True), 1e-3)
        n = self.y_shape[0]
        ncols = n
        lo = channels.read_layout(sensor_layout)
        # lo = channels.generate_2d_layout(lo.pos)
        ...
        fake_times = np.arange(0, n, 1.0)
        ft = self.fake_evoked.plot_topomap(
            times=fake_times,
            # axes=ax[0, 0],
            colorbar=True,
            # vmax=vmax,
...
class VARCNN(BaseModel):
    ...
    def build_graph(self):
        ...
        dropout = Dropout(self.specs["dropout"], noise_shape=None)(self.pooled)
...
class VARCNNR(BaseModel):
    ...
    def build_graph(self):
        ...
        self.fc1 = Dense(
            size=self.specs["n_latent"] * 2,
            nonlin=self.specs["nonlin"],
            specs=self.specs,
        )(self.pooled)

        dropout = Dropout(self.specs["dropout"], noise_shape=None)(self.fc1)
...
class FBCSP_ShallowNet(BaseModel):
    ...
    def build_graph(self):
        ...
        print(
            "tconv1: ", tconv1_out.shape
        )  # should be n_batch, sensors, times, kernels
...
class LFLSTM(BaseModel):
    ...
    def build_graph(self):
        ...
        ffeatures = tf.reshape(pooled, [-1, self.dataset.h_params["n_seq"], fshape])
        ...
        if self.return_sequence == True:
            self.fin_fc = DeMixing(size=self.out_dim, nonlin=tf.identity, axis=2)
        else:
...
class EEGNet(BaseModel):
    ...
    def build_graph(self):
        ...
        block2 = SeparableConv2D(
            self.specs["n_latent"] * self.specs["depth_multiplier"],
...
class SimpleNet(LFCNN):
    ...
    def compute_patterns(self, data_path=None, *, output="patterns"):
        ...
        self.out_weights = np.reshape(
            self.out_w_flat, [-1, self.dmx.size, self.out_dim]
        )
    ...
    def plot_patterns(
        ...
        if info:
            ...
            for i, ch in enumerate(lo.names):
                if info["chs"][i]["ch_name"] == ch:
                    info["chs"][i]["loc"][:2] = (lo.pos[i, :2] - orig_xy) / 3.0
                    # info['chs'][i]['loc'][4:] = 0
                else:
                    print(
                        "Channel name mismatch. info: {} vs lo: {}".format(
        ...
        if np.any(self.uorder):
            ...
            for ii in range(nrows):
                fake_times = np.arange(ii * ncols, (ii + 1) * ncols, 1.0)
    ...
    def plot_branch(
        ...
        relevance = sorted(
            [np.round(rel / relevances.sum(), 2) for rel in relevances], reverse=True
        )[branch_num]
